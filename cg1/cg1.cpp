...
void hexLineFractal(Turtle &t, float distance, int recursionLeft, bool left) {
    if (recursionLeft == 0) {
        t.draw(distance);
        return;
    }
    if (left) {
        // Left branch
        t.rotate(pi / 3); 
        hexLineFractal(t, distance / 2, recursionLeft - 1, false);
        t.rotate(-pi / 3);
        hexLineFractal(t, distance / 2, recursionLeft - 1, true);
        t.rotate(-pi / 3);
        hexLineFractal(t, distance / 2, recursionLeft - 1, false);
        t.rotate(pi / 3);
    } else {
...
void Display4() {
  ...
  Turtle t0(-0.75, -0.5);
  float length = 1.5; // lungimea segm init

  hexLineFractal(t0, length, g_recursionCurrent, true);
